...
void laneChange(const int direction, queue<double> &lane_change_offsets, int &current_lane) {
  ...
  vector<double> jmt = JMT(current_position_horiz, next_position_horiz, 4.0);

  for (double t = .02; t <= 4.0; t += .02) {
    double offset = 0;
...
int main() {
  ...
  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy, &middle_line_trajectory_x, &middle_line_trajectory_y, &trajectory_points_inserted, &lane_change_offsets, &transition_speeds, &current_lane, &max_speed, &goal_speed](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
	uWS::OpCode opCode) {
      ...
      // Calculating data at the end of the path so we can make predictions
	...
	end_path_theta = atan2(end_path_y - second_end_path_y, end_path_x - second_end_path_x);
      ...
      const int num_waypoints = map_waypoints_x.size();

      // Update the middle line by taking away the points we've traveled.
	...
	trajectory_points_inserted--;
      ...
      // Push the old path back so it doesn't change
	...
	next_y_vals.push_back(previous_path_y[i]);
      ...
      vector<vector<double>> left_cars, right_cars, front_cars;
	...
	other_car_speed *= (seconds_per_hour / meters_per_mile); // Convert from meters per secont to mph

	double distance = (double)sensor_fusion[i][5] - car_s;
	int other_car_lane = (other_car_d / 4);

	if (-10 <= distance && distance <= 30 && lane_change_offsets.empty()) {
	  if (other_car_lane == current_lane - 1) left_cars.push_back(sensor_fusion[i]);
	  else if (other_car_lane == current_lane) front_cars.push_back(sensor_fusion[i]);
	  else if (other_car_lane == current_lane + 1) right_cars.push_back(sensor_fusion[i]);
	}
      }

      // If there's a car in front of us, take action. Switch lanes or slow down.
      if (!front_cars.empty()) {
	if (right_cars.empty() && current_lane < 2) laneChange(RIGHT, lane_change_offsets, current_lane);
	else if (left_cars.empty() && current_lane > 0) laneChange(LEFT, lane_change_offsets, current_lane);
	else {
	  // Find the closest car in front of us
	  vector<double> closest_car = front_cars[0];
	  for (int i = 1; i < front_cars.size(); i++)
	    if (front_cars[i][5] < closest_car[5]) closest_car = front_cars[i];

	  //  Match closest car's speed
	  double closest_car_theta = atan2(closest_car[4], closest_car[3]);
	  double closest_car_vx = closest_car[3];
	  double closest_car_vy = closest_car[4];
	  double closest_car_speed = closest_car_vx * cos(-closest_car_theta) - closest_car_vy * sin(-closest_car_theta);
	  closest_car_speed *= (seconds_per_hour / meters_per_mile);

	  for (double speed = goal_speed; speed > closest_car_speed; speed -= .05)
	    if (speed < max_speed)
	      transition_speeds.push(speed);

	  goal_speed = min(closest_car_speed, max_speed);
	}
      } else {
	// Speed up to max speed
	for (double speed = goal_speed; speed < max_speed; speed += .05) {
  	  transition_speeds.push(speed);
	}
	goal_speed = max_speed;
      }

      // Spline starts 4 waypoints behind the current one and ends 4 waypoints ahead
      int starting_waypoint = ClosestWaypoint(end_path_x, end_path_y, map_waypoints_x, map_waypoints_y) - 4;
      // Make sure that starting waypoint is within the number of waypoints that we have.
      starting_waypoint = starting_waypoint % num_waypoints;
      starting_waypoint += (starting_waypoint < 0) * num_waypoints;

      // Set up our spline for the new path
      tk::spline s;
	...
	// Shift the points so that we are at the origin
	// This helps to avoid errors when the line goes straight up
	// as vertical lines aren't represented well in splines
	double shifted_x = map_waypoints_x[waypoint_num] - end_path_x;
	double shifted_y = map_waypoints_y[waypoint_num] - end_path_y;

	// Rotate the points so that the x axis is ahead of us
	...
	spline_y.push_back(rotated_y);
      ...
      // Now that we have our spline set up, add however many points we need to get to 100
      for (int i = 1; i <= 100 - previous_path_length; i++) {
	// Go the goal speed if there's nothing on the transition queue,
	// otherwise take the latest value from the transition queue
	double speed = goal_speed;
	if (!transition_speeds.empty()) {
	  speed = transition_speeds.front();
	  transition_speeds.pop();
	}

	// Find how far from the center line we want to be
	// If there are offsets in the lane change queue, we use them
	double center_offset = current_lane * 4 + 2;
	double next_offset = current_lane * 4 + 2;
	if (!lane_change_offsets.empty()) {
	  center_offset = lane_change_offsets.front();
	  lane_change_offsets.pop();
	  if (!lane_change_offsets.empty()) next_offset = lane_change_offsets.front();
	}
	double sideways_velocity = abs(center_offset - next_offset)/.02;

	double forward_velocity;
	if (sideways_velocity > speed) forward_velocity = 0;
	else forward_velocity = sqrt(speed*speed - sideways_velocity*sideways_velocity);

	// Calculate how far to go on this iteration
	const double seconds_per_iteration = .02;
	const double speed_meters_per_second = forward_velocity * meters_per_mile / seconds_per_hour;
	const double speed_meters_per_iteration = speed_meters_per_second * seconds_per_iteration;

	// Get the next x,y along the spline in spline coordinates
	double rotated_x = speed_meters_per_iteration * i;
	double rotated_y = s(rotated_x);
	double rotated_y_offset = rotated_y - center_offset;

	// Rotate and shift everything to global coordinates instead of local
